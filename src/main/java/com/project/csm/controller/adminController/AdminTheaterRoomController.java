...
public class AdminTheaterRoomController {
	...
	public String showRooms(@PathVariable Long theaterID, Model model, HttpSession session) {
	...
	public String getCreateTheaterRoom(@PathVariable Long theaterID, Model model, HttpSession session) {
		Account account = (Account) session.getAttribute("accountLoggedIn");
		if (account == null || !account.getRole().equals("admin")) {
			return "redirect:/";
		}
